...
window.UI = {
  init: function () {
    ...
    $('body')
      .on('click', '.dosplit:not(.disabled)', function (e) {
        e.preventDefault()
  ...
  progressBar: function (perc) {
    ...
    $('#longloading .approved-bar').attr(
      'title',
      'Analyzing ' + parseInt(perc * 100) + '%',
    )
    ...
    return APP.doRequest({
      ...
      success: function (d) {
        if (d.data) {
          ...
          if (s.STATUS == 'NEW' || s.STATUS == '' || s.IN_QUEUE_BEFORE > 0) {
            ...
            if (config.daemon_warning) {
              ...
              if (-1 == config.support_mail.indexOf('@')) {
                ...
                analyzerNotRunningErrorString =
                  'The analysis seems not to be running. Contact <a href="mailto:' +
                  config.support_mail +
                  '">' +
                  config.support_mail +
                  '</a>.'
              ...
              $('#standard-equivalent-words .word-number')
                .removeClass('loading')
                .text($('#raw-words .word-number').text())
              $('#matecat-equivalent-words .word-number')
                .removeClass('loading')
                .text($('#raw-words .word-number').text())
              return false
              ...
              if (UI.previousQueueSize <= s.IN_QUEUE_BEFORE) {
                $('#shortloading')
                  .show()
                  .html(
                    '<p class="label">There are other projects in queue. Please wait...</p>',
                  )
                $('#longloading').hide()
              } else {
                ...
                if (!$('#shortloading .queue').length) {
                  $('#shortloading').html(
                    '<p class="label">There are still <span class="number">' +
                      s.IN_QUEUE_BEFORE_PRINT +
                      '</span> segments in queue. Please wait...</p>',
                  )
                } else {
                  $('#shortloading .queue .number').text(
                    s.IN_QUEUE_BEFORE_PRINT,
                  )
                }
              }
          ...
          if (
            old_standard_words != s.TOTAL_STANDARD_WC_PRINT &&
            old_standard_words != ''
          )
          ...
          if (
            old_matecat_words != s.TOTAL_PAYABLE_PRINT &&
            old_matecat_words != ''
          )
            ...
            $.each(d.data.jobs, function (job_id, group) {
              ...
              $.each(total_group, function (jPassword, tot) {
                context = $(global_context)
                  .find(".tablestats[data-pwd='" + jPassword + "']")
                  .find('.totaltable')
              ...
              $.each(files_group, function (jPassword, files_object) {
                $.each(files_object, function (id_file, file_details) {
                  context = $(global_context).find(
                    '#file_' + job_id + '_' + jPassword + '_' + id_file,
                  )
          ...
          if (d.data.summary.STATUS == 'DONE') {
            ...
            setTimeout(function () {
              ...
              $('#longloading p')
                .addClass('loaded')
                .html('<span class="complete">Analysis complete</span>')
                .append(
                  '<a class="downloadAnalysisReport">Download Analysis Report</a>',
                )
            ...
            sew
              .find('.word-number')
              .removeClass('loading')
              .text(rwc.find('.word-number').text())
            ...
            $('#longloading p')
              .addClass('loaded')
              .html('<span class="complete">This job is too big.</span>')
              .append(
                '<span class="analysisNotPerformed">The analysis was not performed.</span>',
              )
            $('.splitbtn').removeClass('disabled').attr('title', '')
          } else {
  ...
  updateSplitPopup: function (button) {
    ...
    $('.popup-split .popup-split-job-title').text(
      jobContainer.find('h3').text(),
    )
    $('.popup-split .popup-split-job-id').text(
      '(' + jobContainer.find('.languages .splitnum').text() + ')',
    )
    $('.popup-split .popup-split-project-title').text(
      $('#pid #pname').data('originaltext'),
    )
    ...
    $('.popup-split .total .total-w')
      .attr('data-val', total)
      .text(APP.addCommas(total))
    ...
    if (jobId && open && job$) {
      switch (open) {
        case 'analysis':
          ...
          setTimeout(function () {
            $('div[data-jid=' + jobId + '] .uploadbtn.translate').trigger(
              'click',
            )
          }, 500)
          break
  ...
  updateJobPassword: function (password) {
    ...
    $job
      .find('.uploadbtn.translate')
      .attr('href', href.replace(UI.currentOutsourceJob.password, password))
