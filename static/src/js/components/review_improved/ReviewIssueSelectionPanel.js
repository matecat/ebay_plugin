...
const ReviewIssueCategorySelector =
  require('./ReviewIssueCategorySelector').default
class ReviewIssueSelectionPanel extends React.Component {
  ...
  severitySelected(category, event) {
    ...
    var selections = clone(this.state.selections)
  ...
  render() {
    ...
    return (
      <div className="review-issue-selection-panel">
        <h3>Error selection</h3>

        <p>
          You selected "
          <span className="error-selection-highlight">
            {this.props.selection.selected_string}
          </span>
          " from segment {this.props.sid}
        </p>
        <h4>Select issue type</h4>
        <table className="review-issue-category-list">
          <tbody>{categoryComponents}</tbody>
        </table>
