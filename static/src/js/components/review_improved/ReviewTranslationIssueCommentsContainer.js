...
class ReviewTranslationIssueCommentsContainer extends React.Component {
    ...
    ReviewImproved.undoRebutIssue(this.props.sid, this.props.issueId)
      ...
      .fail(
        function () {
          this.setState({
            undoRebutDisabled: false,
            undoRebutLabel: this.getInitialState().undoRebutLabel,
          })
        }.bind(this),
      )
  }
  ...
  componentDidMount() {
    ...
    MateCat.db.addListener(
      ...
      ['insert', 'update'],
      this.issueChanged.bind(this),
    )
  }
  componentWillUnmount() {
    MateCat.db.removeListener(
      'segment_translation_issue_comments',
      ['insert', 'delete'],
  ...
  sendClick() {
    ...
    SegmentActions.submitComment(this.props.sid, this.props.issueId, data).fail(
      this.handleFail,
    )
  }
  ...
  render() {
    ...
    var sortedComments = sortBy(this.state.comments, function (comment) {
    ...
    var commentLines = sortedComments.map(function (comment, index) {
      ...
      if (comment.source_page == '1') {
        source_page = (
          <strong className="review-issue-translator">Translator:</strong>
        )
      } else {
        source_page = (
          <strong className="review-issue-revisor">Reviewer:</strong>
        )
      }

      return (
        <div key={comment.id} className="review-issue-comment-detail">
    ...
    if (!this.state.replying) {
      ...
      if (
        ...
        undoRebutButton = (
          <a
            onClick={this.undoRebutClick.bind(this)}
            className={undoRebutButtonClasses}
          >
      ...
      terminal = (
        <div className="review-issue-comment-reply">
          ...
          <div className="review-issue-comment-buttons">
            <div className="review-issue-comment-buttons-right">
              {rebutButton}
              <a onClick={this.sendClick.bind(this)} className={buttonClasses}>
    ...
    return (
      <div
        ...
        className="review-issue-comment-container"
      >
        <div className="review-issue-comment-entries">{commentLines}</div>
        <hr />
        {terminal}
      </div>
    )
