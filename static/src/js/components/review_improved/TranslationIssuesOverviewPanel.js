...
class TranslationIssuesOverviewPanel extends React.Component {
  ...
  getListVersionsReviewImproved() {
    let versions = this.getVersions(this.props.sid)
    let segment = MateCat.db.segments.by('sid', this.props.sid)
    let previousVersions = versions.map(
      function (v) {
  ...
  render() {
    ...
    return (
      <div className="review-issues-overview-panel">
        <div className="review-version-wrapper">
          <h3>Original target</h3>
          <div
            className="ui ignored message"
            dangerouslySetInnerHTML={this.originalTarget()}
          />
          <div
            className="review-side-panel-close"
            onClick={this.closePanelClick.bind(this)}
          >
            x
          </div>
        </div>
