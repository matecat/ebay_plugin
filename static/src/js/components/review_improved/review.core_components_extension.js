;(function () {
  let ReviewImprovedSideButton =
    require('./ReviewImprovedTranslationIssuesSideButton').default
      ...
      if (this.state.showStatusMenu) {
        return (
          <ul
            ...
            ref={(menu) => (this.statusMenuRef = menu)}
          >
            <li className="arrow">
  ...
  function overrideTargetButtons(SegmentButtons) {
    ...
    SegmentButtons.prototype.getReviewImprovedButtons = function () {
      let button
      let currentScore = ReviewImproved.getLatestScoreForSegment(
    ...
    SegmentButtons.prototype.getReviewImprovedTranslateButtons = function () {
      //TODO Remove lokiJs
      let data = MateCat.db.segments.by('sid', this.props.segment.original_sid)
